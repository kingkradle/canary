...
def main():
    ...
    df = pd.read_csv('techniques.csv')

    # Select and prepare columns
    df = df[["ID", "name", "description", "url", "domain", "tactics", "is sub-technique", "sub-technique of"]]
    df['tactics'] = df['tactics'].apply(lambda x: x.split(',') if ',' in x else [x])

    # Split into parent and child dataframes
    df_parent = df[df["is sub-technique"] == False][["ID", "name", "description", "url", "domain", "tactics"]].rename(columns={"ID": "technique_id"})
    df_child = df[df["is sub-technique"] == True][["ID", "name", "description", "url", "domain", "tactics", "sub-technique of"]].rename(columns={"ID": "technique_id", "sub-technique of": "parent_id"})
    ...
    # Save to CSV files
    df_parent.to_csv('parent.csv', index=False)
    df_tactics.to_csv('tactics.csv', index=False)
