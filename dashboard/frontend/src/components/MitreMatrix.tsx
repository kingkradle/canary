...
interface AttacksMap {
  [attackId: string]: string[]
}

const MITREAttackMatrix = () => {
  const [techniques, setTechniques] = useState<TechniquesByTactic>({})
  const [attacks, setAttacks] = useState<AttacksMap>({})
  const [selectedAttackId, setSelectedAttackId] = useState<string>('')
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Fallback map for tactic shortnames to full names
  const tacticNameMap: { [key: string]: string } = {
  ...
  const tacticUrlMap: { [key: string]: string } = {
  ...
  const parseCSV = (csvText: string): AttacksMap => {
    ...
    const attacksMap: AttacksMap = {}
    for (let i = 1; i < lines.length; i++) {
      const line = lines[i].trim()
      if (line && !line.startsWith('created_at')) {
  ...
  const fetchAttackMapping = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/mitre-campaigns-csv`)
      if (!response.ok) {
  ...
  useEffect(() => {
    const fetchMitreData = async () => {
      try {
        ...
        const tacticShortnameToName: { [key: string]: string } = {}
        ...
        // Parse the data
        const techniquesByTactic: TechniquesByTactic = {}
        ...
        json.objects.forEach((obj: any) => {
          ...
          if (obj.type === 'attack-pattern' && !obj.revoked && !obj.x_mitre_deprecated) {
            ...
            const mitreRef = externalRefs.find((ref: any) => ref.source_name === 'mitre-attack')
            ...
            if (mitreRef && mitreRef.external_id) {
              ...
              const mitreTactics = killChainPhases
                .filter((phase: any) => phase.kill_chain_name === 'mitre-attack')
                .map((phase: any) => {
              ...
              mitreTactics.forEach((tactic: string) => {
  ...
  // Generate heatmap color based on frequency
  const getHeatmapColor = (techniqueId: string): string => {
